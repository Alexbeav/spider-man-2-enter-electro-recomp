...
#
# Writes: dist/spiderman2enterelectro-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix spiderman2enterelectro \
  --exe-name Spider_Man_2_Enter_Electro \
  --omit-openbios \
  --display-name "Spider-Man 2: Enter Electro" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Spider-Man 2: Enter Electro (USA) disc" \
  --runtime-dir mods \
